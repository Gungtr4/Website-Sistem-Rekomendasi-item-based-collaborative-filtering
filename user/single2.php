...
if(isset($_POST['submit'])){
	...
	$sql	= 'INSERT INTO order_detail (id_detail,id_order,id_produk,jumlah,total_harga,status) VALUES ("'.$idt.'","'.$ido.'","'.$id_p.'","'.$jumlah.'","'.$tharga.'","waiting")';	
	$ins   = mysqli_query($koneksi,$sql) or die (mysqli_error($koneksi));
		if($ins){
				echo'
				<script>
					alert("Proses Pembayaran ");
					window.location="bayar.php?kode='.$_GET['kode'].'&order='.$ido.'";
				</script>';
			}else{
				echo'
					<script>
